...
void HMC5883L_WriteByte(uint8_t _ucRegAddr, uint8_t _ucRegData)
{
    ...
    IIC_Send_Byte(HMC5883L_SLAVE_ADDRESS);	/* 发送设备地址+写信号 */
	IIC_Wait_Ack();

    IIC_Send_Byte(_ucRegAddr);				/* 内部寄存器地址 */
	IIC_Wait_Ack();

    IIC_Send_Byte(_ucRegData);				/* 内部寄存器数据 */
	IIC_Wait_Ack();
...
uint8_t HMC5883L_ReadByte(uint8_t _ucRegAddr)
{
	...
	IIC_Send_Byte(HMC5883L_SLAVE_ADDRESS);	/* 发送设备地址+写信号 */
	IIC_Wait_Ack();
	IIC_Send_Byte(_ucRegAddr);     			/* 发送存储单元地址 */
	IIC_Wait_Ack();

	IIC_Start();                  			/* 总线开始信号 */

	IIC_Send_Byte(HMC5883L_SLAVE_ADDRESS+1); 	/* 发送设备地址+读信号 */
	IIC_Wait_Ack();

	ucData = IIC_Read_Byte(0);       			/* 读出寄存器数据 */
	IIC_NAck();
	IIC_Stop();                  			/* 总线停止信号 */
	return ucData;
}
...
void HMC5883L_ReadData(void)
{
	...
	IIC_Send_Byte(HMC5883L_SLAVE_ADDRESS);	/* 发送设备地址+写信号 */
	IIC_Wait_Ack();
	IIC_Send_Byte(DATA_OUT_X);     		/* 发送存储单元地址  */
	IIC_Wait_Ack();
	...
	for (i = 0; i < 6; i++)
	{
		ucReadBuf[i] = IIC_Read_Byte(1);       			/* 读出寄存器数据 */
//		IIC_Ack();
	}
