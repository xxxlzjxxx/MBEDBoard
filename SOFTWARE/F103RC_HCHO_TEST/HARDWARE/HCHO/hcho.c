...
//接收缓存区 	
u8 HCHO_RX_BUF[64];  	//接收缓冲,最大64个字节.
//接收到的数据长度
u8 HCHO_RX_CNT=0;   		  
...
void USART2_IRQHandler(void)
{
	...
	{	 
		...
		if(HCHO_RX_CNT<64)
		{
			HCHO_RX_BUF[HCHO_RX_CNT]=res;		//记录接收到的值
			HCHO_RX_CNT++;						//接收数据增加1 
		} 
	}  											 
} 
...
void HCHO_Receive_Data(u8 *buf, u8 *len)
{
	...
	*len = 0;				//默认为0
	...
	if(rxlen == HCHO_RX_CNT && rxlen)//接收到了数据,且接收完成了
	{
		for(i=0;i<rxlen;i++)
		{
			buf[i] = HCHO_RX_BUF[i];	
		}		
		*len=HCHO_RX_CNT;	//记录本次数据长度
		HCHO_RX_CNT=0;		//清零
	}
}
